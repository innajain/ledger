...
export default function ClientPage({
  ...
  const [openingBalances, setOpeningBalances] = useState<OpeningBalanceState[]>(() => {
    return (initial_data.opening_balances || []).map((ob: any) => ({
      ...
      allocations: (ob.allocation_to_purpose_buckets || []).map((a: any) => ({
        id: a.id,
  ...
  function addAllocation(obIndex: number) {
    setOpeningBalances(prev =>
      prev.map((ob, i) =>
        i === obIndex ? { ...ob, allocations: [...ob.allocations, { purpose_bucket_id: initial_buckets[0]?.id || '', quantity: 0 }] } : ob
      )
    );
  }
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      for (const ob of openingBalances) {
        const allocSum = ob.allocations.filter(a => !a._deleted).reduce((s, a) => s.plus(new Decimal(a.quantity || 0)), new Decimal(0));
        if (!new Decimal(ob.quantity).equals(allocSum))
          throw new Error(`Opening balance for asset ${ob.asset_id} must equal sum of allocations (${allocSum.toNumber()})`);
      }

      const creates = openingBalances
        .filter(ob => !ob.id)
        .map(ob => ({
          asset_id: ob.asset_id,
          quantity: new Decimal(ob.quantity).toNumber(),
          date: ob.date || getCurrentIndianDate(),
          allocation_to_purpose_buckets: ob.allocations
            .filter(a => !a.id)
            .filter(a => !a._deleted)
            .map(a => ({ purpose_bucket_id: a.purpose_bucket_id, quantity: new Decimal(a.quantity).toNumber() })),
        }));

      const updates = openingBalances
        .filter(ob => ob.id)
        .map(ob => ({
          id: ob.id as string,
          asset_id: ob.asset_id,
          quantity: new Decimal(ob.quantity).toNumber(),
          date: ob.date || getCurrentIndianDate(),
          allocation_to_purpose_buckets_creates: ob.allocations
            .filter(a => !a.id && !a._deleted)
            .map(a => ({ purpose_bucket_id: a.purpose_bucket_id, quantity: new Decimal(a.quantity).toNumber() })),
          allocation_to_purpose_buckets_deletes: ob.allocations.filter(a => a.id && a._deleted).map(a => ({ id: a.id! })),
          allocation_to_purpose_buckets_updates: ob.allocations
            .filter(a => a.id && !a._deleted)
            .map(a => ({ id: a.id!, purpose_bucket_id: a.purpose_bucket_id, quantity: new Decimal(a.quantity).toNumber() })),
        }));

      const deletes = openingBalanceDeletes;

      await update_account(initial_data.id, {
        name,
        opening_balance_creates: creates,
        opening_balance_updates: updates,
        opening_balance_deletes: deletes,
      });
        ...
        <form onSubmit={onSubmit} className="space-y-6">
          ...
          <section>
            <div className="flex items-center justify-between">
              ...
              <button type="button" onClick={addOpeningBalance} className="px-3 py-1 bg-emerald-500 text-white rounded">
                Add Opening Balance
              </button>
            </div>
                ...
                <div key={ob.id || `new-${i}`} className="p-3 border rounded">
                  <div className="grid grid-cols-4 gap-3 items-end">
                    <div>
                      ...
                      <select
                        value={ob.asset_id}
                        onChange={e => setOpeningBalances(prev => prev.map((p, idx) => (idx === i ? { ...p, asset_id: e.target.value } : p)))}
                    ...
                    <div>
                      ...
                      <input
                        type="number"
                        step="any"
                        value={ob.quantity as any}
                        onChange={e => setOpeningBalances(prev => prev.map((p, idx) => (idx === i ? { ...p, quantity: Number(e.target.value) } : p)))}
                        className="mt-1 block w-full border rounded px-2 py-1"
                      />
                    </div>
                    <div>
                      ...
                      <input
                        type="text"
                        value={ob.date}
                        onChange={e => setOpeningBalances(prev => prev.map((p, idx) => (idx === i ? { ...p, date: e.target.value } : p)))}
                        placeholder="dd-mm-yyyy"
                        className="mt-1 block w-full border rounded px-2 py-1"
                      />
                    </div>
                    <div className="text-right">
                      <button type="button" onClick={() => removeOpeningBalance(i)} className="px-3 py-1 bg-red-500 text-white rounded">
                        Remove
                      </button>
                    </div>
                  </div>
                  ...
                  <div className="mt-3">
                    <div className="flex items-center justify-between">
                      ...
                      <button type="button" onClick={() => addAllocation(i)} className="text-sm px-2 py-1 bg-emerald-200 rounded">
                        Add Allocation
                      </button>
                    </div>
                    <div className="mt-2 space-y-2">
                      {ob.allocations.map((a, ai) =>
                        a._deleted ? null : (
                          <div key={a.id || `na-${ai}`} className="grid grid-cols-3 gap-2 items-end">
                            <div>
                              <label className="block text-xs">Bucket</label>
                              <select
                                value={a.purpose_bucket_id}
                                onChange={e =>
                                  setOpeningBalances(prev =>
                                    prev.map((p, idx) =>
                                      idx === i
                                        ? {
                                            ...p,
                                            allocations: p.allocations.map((aa, aidx) =>
                                              aidx === ai ? { ...aa, purpose_bucket_id: e.target.value } : aa
                                            ),
                                          }
                                        : p
                                    )
                                    ...
                                    prev.map((p, idx) =>
                                      idx === i
                                        ? {
                                            ...p,
                                            allocations: p.allocations.map((aa, aidx) =>
                                              aidx === ai ? { ...aa, quantity: Number(e.target.value) } : aa
                                            ),
                                          }
                                        : p
                                    )
                            ...
                            <div className="text-right">
                              <button type="button" onClick={() => removeAllocation(i, ai)} className="px-2 py-1 bg-red-500 text-white rounded">
                                Remove
                              </button>
                            </div>
                          </div>
          ...
          <div className="flex gap-2">
            <button type="submit" disabled={saving} className="px-4 py-2 bg-orange-500 text-white rounded">
              Save
            </button>
            <a href={`/accounts/${initial_data.id}`} className="px-4 py-2 border rounded">
              Cancel
            </a>
          </div>
        </form>
