...
export default function ClientPage({ initial_data, initial_buckets, initial_assets, initial_accounts }: { initial_data: any; initial_buckets: { id: string; name: string }[]; initial_assets: { id: string; name: string }[]; initial_accounts: { id: string; name: string }[] }) {
  ...
  const initialIncomeAllocs: Allocation[] = (txn.income_txn?.allocation_to_purpose_buckets || []).map((a: any) => ({ id: a.id, purpose_bucket_id: a.purpose_bucket_id, quantity: a.quantity }));
  const [incomeAllocs, setIncomeAllocs] = useState<Allocation[]>(initialIncomeAllocs);

  const initialAssetReplacements: AssetReplacement[] = (txn.asset_trade_txn?.asset_replacement_in_purpose_buckets || []).map((a: any) => ({ id: a.id, purpose_bucket_id: a.purpose_bucket_id, debit_quantity: a.debit_quantity, credit_quantity: a.credit_quantity }));
  ...
  const [expensePurposeBucketId, setExpensePurposeBucketId] = useState<string | undefined>(txn.expense_txn?.purpose_bucket_id ?? initial_buckets[0]?.id);
  ...
  const [stFromAccountId, setStFromAccountId] = useState<string | undefined>(txn.self_transfer_or_refundable_or_refund_txn?.from_account_id ?? initial_accounts[0]?.id);
  const [stToAccountId, setStToAccountId] = useState<string | undefined>(txn.self_transfer_or_refundable_or_refund_txn?.to_account_id ?? initial_accounts[0]?.id);
  ...
  const datePayload = dateStr ? dateStr : undefined;
      ...
      if (txn.income_txn) {
        ...
        const creates = incomeAllocs.filter(a => !a.id && !a.markedForDelete).map(a => ({ purpose_bucket_id: a.purpose_bucket_id, quantity: Number(a.quantity) }));
        const updates = incomeAllocs.filter(a => a.id && !a.markedForDelete).map(a => ({ id: a.id!, purpose_bucket_id: a.purpose_bucket_id, quantity: Number(a.quantity) }));
        ...
        await update_income_transaction(txn.id, { description, date: datePayload, asset_id: incomeAssetId, account_id: incomeAccountId, quantity: incomeQty, allocation_to_purpose_buckets_creates: creates, allocation_to_purpose_buckets_deletes: deletes, allocation_to_purpose_buckets_updates: updates });
      } else if (txn.expense_txn) {
        await update_expense_transaction(txn.id, { description, date: datePayload, asset_id: expenseAssetId, account_id: expenseAccountId, quantity: Number(expenseQuantity), purpose_bucket_id: expensePurposeBucketId });
      } else if (txn.self_transfer_or_refundable_or_refund_txn) {
        await update_self_transfer_or_refundable_or_refund_transaction(txn.id, { type: stType as any, description, date: datePayload, asset_id: stAssetId, from_account_id: stFromAccountId, to_account_id: stToAccountId, quantity: Number(stQuantity) });
      } else if (txn.asset_trade_txn) {
        const creates = assetReplacements.filter(a => !a.id && !a.markedForDelete).map(a => ({ purpose_bucket_id: a.purpose_bucket_id, debit_quantity: Number(a.debit_quantity), credit_quantity: Number(a.credit_quantity) }));
        const updates = assetReplacements.filter(a => a.id && !a.markedForDelete).map(a => ({ id: a.id!, purpose_bucket_id: a.purpose_bucket_id, debit_quantity: Number(a.debit_quantity), credit_quantity: Number(a.credit_quantity) }));
        ...
        await update_asset_trade_transaction(txn.id, { description, debit_asset_id: debitAssetId, debit_account_id: debitAccountId, debit_quantity: Number(debitQuantity), debit_date: datePayload, credit_asset_id: creditAssetId, credit_account_id: creditAccountId, credit_quantity: Number(creditQuantity), credit_date: datePayload, asset_replacement_in_purpose_buckets_creates: creates, asset_replacement_in_purpose_buckets_deletes: deletes, asset_replacement_in_purpose_buckets_updates: updates });
      }
            ...
            <div className="space-y-2">
              <div className="grid grid-cols-3 gap-2">
                <div>
                  ...
                  <select value={incomeAssetId} onChange={e => setIncomeAssetId(e.target.value)} className="mt-1 block w-full border rounded px-2 py-1">
                    {initial_assets.map((a: Asset) => <option key={a.id} value={a.id}>{a.name}</option>)}
                  </select>
                </div>
                <div>
                  <label className="block text-sm">Account</label>
                  <select value={incomeAccountId} onChange={e => setIncomeAccountId(e.target.value)} className="mt-1 block w-full border rounded px-2 py-1">
                    {initial_accounts.map((ac: Account) => <option key={ac.id} value={ac.id}>{ac.name}</option>)}
                  </select>
                </div>
                <div>
                  <label className="block text-sm">Quantity</label>
                  <input type="number" value={incomeQuantity} onChange={e => setIncomeQuantity(Number(e.target.value))} className="mt-1 block w-full border rounded px-2 py-1" />
                </div>
              </div>
            </div>
          )}
          ...
          {txn.expense_txn && (
            <div className="space-y-2">
              <div className="grid grid-cols-4 gap-2">
                <div>
                  <label className="block text-sm">Asset</label>
                  <select value={expenseAssetId} onChange={e => setExpenseAssetId(e.target.value)} className="mt-1 block w-full border rounded px-2 py-1">
                    {initial_assets.map((a: Asset) => <option key={a.id} value={a.id}>{a.name}</option>)}
                  </select>
                </div>
                <div>
                  <label className="block text-sm">Account</label>
                  <select value={expenseAccountId} onChange={e => setExpenseAccountId(e.target.value)} className="mt-1 block w-full border rounded px-2 py-1">
                    {initial_accounts.map((ac: Account) => <option key={ac.id} value={ac.id}>{ac.name}</option>)}
                  </select>
                </div>
                <div>
                  <label className="block text-sm">Quantity</label>
                  <input type="number" value={expenseQuantity} onChange={e => setExpenseQuantity(Number(e.target.value))} className="mt-1 block w-full border rounded px-2 py-1" />
                </div>
                <div>
                  <label className="block text-sm">Bucket</label>
                  <select value={expensePurposeBucketId} onChange={e => setExpensePurposeBucketId(e.target.value)} className="mt-1 block w-full border rounded px-2 py-1">
                    {initial_buckets.map((b: Bucket) => <option key={b.id} value={b.id}>{b.name}</option>)}
                  </select>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-2">
              <div className="grid grid-cols-4 gap-2">
                ...
                <div>
                  ...
                  <select value={stFromAccountId} onChange={e => setStFromAccountId(e.target.value)} className="mt-1 block w-full border rounded px-2 py-1">
                    {initial_accounts.map((ac: Account) => <option key={ac.id} value={ac.id}>{ac.name}</option>)}
                  </select>
                </div>
                <div>
                  <label className="block text-sm">To account</label>
                  <select value={stToAccountId} onChange={e => setStToAccountId(e.target.value)} className="mt-1 block w-full border rounded px-2 py-1">
                    {initial_accounts.map((ac: Account) => <option key={ac.id} value={ac.id}>{ac.name}</option>)}
                  </select>
                </div>
              </div>
              <div>
                <label className="block text-sm">Quantity</label>
                <input type="number" value={stQuantity} onChange={e => setStQuantity(Number(e.target.value))} className="mt-1 block w-40 border rounded px-2 py-1" />
              </div>
            </div>
          )}
            ...
            <div className="space-y-2">
              <div className="grid grid-cols-3 gap-2">
                ...
                <div>
                  ...
                  <select value={debitAccountId} onChange={e => setDebitAccountId(e.target.value)} className="mt-1 block w-full border rounded px-2 py-1">
                    {initial_accounts.map((ac: Account) => <option key={ac.id} value={ac.id}>{ac.name}</option>)}
                  </select>
                </div>
                <div>
                  <label className="block text-sm">Debit qty</label>
                  <input type="number" value={debitQuantity} onChange={e => setDebitQuantity(Number(e.target.value))} className="mt-1 block w-full border rounded px-2 py-1" />
                </div>
              </div>
              <div className="grid grid-cols-3 gap-2">
                <div>
                  <label className="block text-sm">Credit asset</label>
                  <select value={creditAssetId} onChange={e => setCreditAssetId(e.target.value)} className="mt-1 block w-full border rounded px-2 py-1">
                    {initial_assets.map((a: Asset) => <option key={a.id} value={a.id}>{a.name}</option>)}
                  </select>
                </div>
                <div>
                  <label className="block text-sm">Credit account</label>
                  <select value={creditAccountId} onChange={e => setCreditAccountId(e.target.value)} className="mt-1 block w-full border rounded px-2 py-1">
                    {initial_accounts.map((ac: Account) => <option key={ac.id} value={ac.id}>{ac.name}</option>)}
                  </select>
                </div>
                <div>
                  <label className="block text-sm">Credit qty</label>
                  <input type="number" value={creditQuantity} onChange={e => setCreditQuantity(Number(e.target.value))} className="mt-1 block w-full border rounded px-2 py-1" />
                </div>
              </div>
            </div>
          )}
              ...
              <div className="mt-2 space-y-2">
                {incomeAllocs.map((alloc, idx) => (
                  <div key={alloc.id ?? `new-${idx}`} className={`flex gap-2 items-center ${alloc.markedForDelete ? 'opacity-50 line-through' : ''}`}>
                    <select value={alloc.purpose_bucket_id} onChange={e => updateIncomeAlloc(idx, { purpose_bucket_id: e.target.value })} className="border rounded px-2 py-1">
                      {initial_buckets.map((b: Bucket) => (
                        <option key={b.id} value={b.id}>{b.name}</option>
                      ))}
                    </select>
                    <input type="number" value={alloc.quantity} onChange={e => updateIncomeAlloc(idx, { quantity: Number(e.target.value) })} className="border rounded px-2 py-1 w-32" />
                    <button type="button" onClick={() => removeIncomeAlloc(idx)} className="px-2 py-1 bg-rose-200 rounded">{alloc.markedForDelete ? 'Undo' : 'Delete'}</button>
                  </div>
                ))}

                <div>
                  <button type="button" onClick={addIncomeAlloc} className="px-3 py-1 bg-sky-200 rounded">Add allocation</button>
                </div>
              </div>
            </div>
          )}
              ...
              <div className="mt-2 space-y-2">
                {assetReplacements.map((r, idx) => (
                  <div key={r.id ?? `new-rep-${idx}`} className={`flex gap-2 items-center ${r.markedForDelete ? 'opacity-50 line-through' : ''}`}>
                    <select value={r.purpose_bucket_id} onChange={e => updateAssetReplacement(idx, { purpose_bucket_id: e.target.value })} className="border rounded px-2 py-1">
                      {initial_buckets.map((b: Bucket) => (
                        <option key={b.id} value={b.id}>{b.name}</option>
                      ))}
                    </select>
                    <input type="number" value={r.debit_quantity} onChange={e => updateAssetReplacement(idx, { debit_quantity: Number(e.target.value) })} className="border rounded px-2 py-1 w-24" placeholder="debit" />
                    <input type="number" value={r.credit_quantity} onChange={e => updateAssetReplacement(idx, { credit_quantity: Number(e.target.value) })} className="border rounded px-2 py-1 w-24" placeholder="credit" />
                    <button type="button" onClick={() => removeAssetReplacement(idx)} className="px-2 py-1 bg-rose-200 rounded">{r.markedForDelete ? 'Undo' : 'Delete'}</button>
                  </div>
                ))}
                <div>
                  <button type="button" onClick={addAssetReplacement} className="px-3 py-1 bg-sky-200 rounded">Add replacement</button>
                </div>
              </div>
            </div>
          )}
