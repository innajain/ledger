        ...
        <div className="flex items-start justify-between">
          ...
          <div className="text-right">
            <div className="flex gap-2 items-center">
              <Link href={`/transactions/${txn.id}/edit`} className="px-3 py-1 border rounded text-sm">
                Edit
              </Link>
            ...
            <div className="p-3 border rounded mt-2">
              <div className="grid grid-cols-2 gap-2">
                <div>
                  ...
                  {txn.income_txn.asset ? (
                    <Link href={`/assets/${txn.income_txn.asset.id}`} className="text-blue-600 hover:underline">
                      {txn.income_txn.asset.name}
                    </Link>
                  ) : (
                    <Link href={`/assets/${txn.income_txn.asset_id}`} className="text-blue-600 hover:underline">
                      {txn.income_txn.asset_id}
                    </Link>
                  )}{' '}
                  {txn.income_txn.asset?.type && <span className="ml-2 text-xs text-gray-500">({txn.income_txn.asset.type})</span>}
                </div>
              ...
              <div className="mt-2">
                ...
                {txn.income_txn.account ? (
                  <Link href={`/accounts/${txn.income_txn.account.id}`} className="text-blue-600 hover:underline">
                ...
                <ul className="mt-2 space-y-1">
                  {(txn.income_txn.allocation_to_purpose_buckets || []).map((a: any) => (
                    <li key={a.id ?? `${a.purpose_bucket_id}-${a.quantity}`} className="flex justify-between">
                      <div>
                        {a.bucket ? (
                          <Link href={`/purpose_buckets/${a.bucket.id}`} className="text-blue-600 hover:underline">
                            {a.bucket.name}
                          </Link>
                        ) : (
                          a.purpose_bucket_id
            ...
            <div className="p-3 border rounded mt-2">
              <div className="grid grid-cols-2 gap-2">
                <div>
                  ...
                  {txn.expense_txn.asset ? (
                    <Link href={`/assets/${txn.expense_txn.asset.id}`} className="text-blue-600 hover:underline">
                      {txn.expense_txn.asset.name}
                    </Link>
                  ) : (
                    <Link href={`/assets/${txn.expense_txn.asset_id}`} className="text-blue-600 hover:underline">
                      {txn.expense_txn.asset_id}
                    </Link>
                  )}{' '}
                  {txn.expense_txn.asset?.type && <span className="ml-2 text-xs text-gray-500">({txn.expense_txn.asset.type})</span>}
                </div>
              ...
              <div className="mt-2">
                ...
                {txn.expense_txn.account ? (
                  <Link href={`/accounts/${txn.expense_txn.account.id}`} className="text-blue-600 hover:underline">
              ...
              <div className="mt-1">
                ...
                {txn.expense_txn.purpose_bucket ? (
                  <Link href={`/purpose_buckets/${txn.expense_txn.purpose_bucket.id}`} className="text-blue-600 hover:underline">
            ...
            <div className="p-3 border rounded mt-2">
              <div className="grid grid-cols-2 gap-2">
                <div>
                  ...
                  {txn.self_transfer_or_refundable_or_refund_txn.asset ? (
                    <Link href={`/assets/${txn.self_transfer_or_refundable_or_refund_txn.asset.id}`} className="text-blue-600 hover:underline">
                      {txn.self_transfer_or_refundable_or_refund_txn.asset.name}
                    </Link>
                  ) : (
                    <Link href={`/assets/${txn.self_transfer_or_refundable_or_refund_txn.asset_id}`} className="text-blue-600 hover:underline">
                      {txn.self_transfer_or_refundable_or_refund_txn.asset_id}
                    </Link>
                  )}{' '}
                  {txn.self_transfer_or_refundable_or_refund_txn.asset?.type && (
                    <span className="ml-2 text-xs text-gray-500">({txn.self_transfer_or_refundable_or_refund_txn.asset.type})</span>
                  )}
                </div>
              ...
              <div className="mt-2">
                ...
                {txn.self_transfer_or_refundable_or_refund_txn.from_account ? (
                  <Link
                    href={`/accounts/${(txn.self_transfer_or_refundable_or_refund_txn.from_account as any).id}`}
              ...
              <div className="mt-1">
                ...
                {txn.self_transfer_or_refundable_or_refund_txn.to_account ? (
                  <Link
                    href={`/accounts/${(txn.self_transfer_or_refundable_or_refund_txn.to_account as any).id}`}
                    className="text-blue-600 hover:underline"
                  >
        ...
        {txn.asset_trade_txn && (
          <section className="mt-4">
            ...
            <div className="p-3 border rounded mt-2">
              <div className="grid grid-cols-2 gap-2">
                <div>
                  Debit: {fmtAmount(txn.asset_trade_txn.debit_quantity, debitAssetType)}{' '}
                  {txn.asset_trade_txn.debit_asset ? (
                    <Link href={`/assets/${txn.asset_trade_txn.debit_asset.id}`} className="text-blue-600 hover:underline">
                      {txn.asset_trade_txn.debit_asset.name}
                    </Link>
                  ) : (
                    <Link href={`/assets/${txn.asset_trade_txn.debit_asset_id}`} className="text-blue-600 hover:underline">
                      {txn.asset_trade_txn.debit_asset_id}
                    </Link>
                  )}{' '}
                  (Account: {txn.asset_trade_txn.debit_account?.name})
                </div>
                <div className="text-right">Credit Date: {fmtDate((txn.asset_trade_txn as any).credit_date)}</div>
              </div>
              <div className="mt-1">
                Credit: {fmtAmount(txn.asset_trade_txn.credit_quantity, creditAssetType)}{' '}
                {txn.asset_trade_txn.credit_asset ? (
                  <Link href={`/assets/${txn.asset_trade_txn.credit_asset.id}`} className="text-blue-600 hover:underline">
                    {txn.asset_trade_txn.credit_asset.name}
                  </Link>
                ) : (
                  <Link href={`/assets/${txn.asset_trade_txn.credit_asset_id}`} className="text-blue-600 hover:underline">
                    {txn.asset_trade_txn.credit_asset_id}
                  </Link>
                )}{' '}
                (Account: {txn.asset_trade_txn.credit_account?.name})
              </div>
              <div className="mt-3">
                    ...
                    <tbody>
                      {(txn.asset_trade_txn.asset_replacement_in_purpose_buckets || []).map((r: any) => (
                        <tr key={r.id ?? `${r.purpose_bucket_id}-${r.debit_quantity}`}>
                          <td className="py-1">
                            {r.purpose_bucket ? (
                              <Link href={`/purpose_buckets/${r.purpose_bucket.id}`} className="text-blue-600 hover:underline">
